...
export default tag CurologyCasePage 
	prop title\string = "Chaitanya Prashant - Curology's subscription frequency"
	...
	<self>
		<div[pos:fixed miw:100% zi:10000 bottom:64px d:flex jc:center rd:999px]>
			<Navbar activeTab="/curology-cadence">
		<page-layout>
			<case-image-hero[hue:purple]>
				<div[d:flex fld:column ai:center w:fit-content p:2]>
					# inner div for tags
					<h2[c:hue9 fw:500 lh:auto margin-block:2px]> "Designing Curology's subscription frequency feature"
				...
				<div[d:flex fld:column ai:center w:100% p:2]>
					<img[rd:xl w:100% maw@md:20%] src=cadence-asset>
				...
				<div[d:flex fld:column ai:center ai@md:flex-start maw:960px]> # case container. Use border to debug. Added max-width 960px here for larger screens 
					...
					<.case-content>
						...
						<div[d:flex fld:column pt:0 w:100%]> # Testing and moving forward
							...
							<ul>
								<li> "This was at the end-state, or when the patient had gone in the flow and completed the change of frequency."
							<p> "The user testing session made clear that we needed to set the correct expectation for how updating the frequency affects a shipment date. We thus knew, that iterating on copy was essential."
							<p> "However, testing validated that the action of finding and changing subscription frequency itself was frictionless and something we could build upon. Task completion, which was optimized for speed, was meeting its desired result in front of the users"
							<h3> "Changes in design from 1st to 2nd user test"
							<div[d:vflex d@md:hflex w:100% p:2]> # image-wrapper container (2 side-by-side)
								# image should have a vflex only container that puts in a caption. 
								<div[d:vflex w:100% ai:center]> 
									<img[rd:xl w:100%] src=case-photos.networks-mvp>
									<p[c:cool5 fw:300 margin-block:12px]> "Copy for 1st user test, after setting change."
								<div[d:vflex w:100% ai:center]> 
									<img[rd:xl w:100%] src=case-photos.networks-mvp>
									<p[c:cool5 fw:300 margin-block:12px]> "Copy for 2nd user test, after setting change."							
							<h3> "Second round of testing"
							<div[d:vflex d@md:hflex w:100% p:2]> # vid-wrapper container
								# image should have a vflex only container that puts in a caption. 
								<div[d:vflex w:100% ai:center]> 
									<video[rd:xl w:100%] autoplay muted loop> # w@md used to focus single image in the center for larger screens.
										<source src="https://chaitanya-portfolio.b-cdn.net/portfolio-2025-image-assets/pagerduty-work/PD-Design-Folder/Iteration-1-prototype.mp4" type="video/mp4">
									<p[c:cool5 fw:300 margin-block:12px]> "Round 2 user test - interactive prototype"
							<p> "On our second round of user tests, we were able to iterate on the end state of our experience. More specifically, the expectation we were setting on the subscription surface, after a frequency setting was changed. In this situation, we set a clear expectation with reactive copy on the modal (“This change only affects boxes shipped after [x date]”)."
							<p> "However, on the frequency card, the header shows the same frequency as previous, since in this scenario, we would only change frequency of the subscription once a patients’ most recent box is shipped. This header copy is contextualized with clear expectation setting, where if a patient has changed their frequency, it says “We’ll ship your box on [current shipment date for recent box], then every 10 weeks after that.”"
							<ul>
								<li> "Use of typography and weight, we hypothesized, would add clarity for our patients in the end state, and the copy itself would provide clear context as to when the changes in setting would propagate."
								<li> "This was done in close collaboration with Tiffany, our copywriter."
							<p> "Whilst the right expectation was set at our end state, 8 out of 10 users that we ran the test on, expected that changing their frequency would affect their most recent, upcoming shipment date. Although all our users, for this test, got their new date right upon completing the flow, it was an interesting insight that our users’ had expected a change in recent shipping date upon changing frequency, as well."

						<div[d:flex fld:column pt:0 w:100%]> # Taking our learnings to craft a final, scalable solution
							<h2> "Taking our learnings to craft a final, scalable solution"
							<div[d:vflex ai:center py:4 rd:xl]> # block-quote
								<div[d:vflex w:100% p:4 ai:center bd:green4/30 rd:sm]>
									<h4[c:cool8 ta:center]> `Before getting down on crafting the final solution, we discussed with the engineers, as well as the PM who launched the initial experiment, that we would intend to use the exact backend logic as the previous experiment to cut down on development time`
							<p> "Whilst the current backend logic was designed to have a condition to make sure that frequency was only being changed after the current shipment date had passed, how it worked was that the logic was changing shipments’ frequency, including the current date, if it was possible to change it."
							<p> "Relative lift of 25% retention in cohort that changed frequency at least once."
							<ul>
								<li> "This version was not launched in the previous experiment, as there wasn't much design work done and the PM raised that it could be quite ambiguous."
							<p> "A clear insight that we had from our two rounds of testing, was that users expected, across two tests, that they would be able to change the closest upcoming shipment date on average."
							<div[d:vflex d@md:hflex w:100% p:2]> # vid-wrapper container
								# image should have a vflex only container that puts in a caption. 
								<div[d:vflex w:100% ai:center]> 
									<video[rd:xl w:100%] autoplay muted loop> # w@md used to focus single image in the center for larger screens.
										<source src="https://chaitanya-portfolio.b-cdn.net/portfolio-2025-image-assets/pagerduty-work/PD-Design-Folder/Iteration-1-prototype.mp4" type="video/mp4">
									<p[c:cool5 fw:300 margin-block:12px]> "Final interactive prototype"
							<p> "To this end, we split our experience, with the constraint of our backend logic, into two discrete paths, one being the happy path, and one being the edge-case scenario."
							<h3> "Happy path design"
							<div[d:vflex d@md:hflex w:100% p:2]> # image-wrapper container
								# image should have a vflex only container that puts in a caption. 
								<div[d:vflex w:100% ai:center]> 
									<img[rd:xl w:100%] src=case-photos.networks-mvp> # w@md used to focus single image in the center for larger screens.
									<p[c:cool5 fw:300 margin-block:12px]> "Scenario - happy path"
							...
							<div[d:vflex d@md:hflex w:100% p:2]> # image-wrapper container
								...
								<div[d:vflex w:100% ai:center]> 
									<img[rd:xl w:100%] src=case-photos.networks-mvp> # w@md used to focus single image in the center for larger screens.
									<p[c:cool5 fw:300 margin-block:12px]> "Scenario - edge case"
							<p> "Scenario: screen date - Sep 1st."
							<ul>
								<li> "Current subscription frequency: 8 weeks. Next billing and shipping date: Sep 8."
								<li> "After changing frequency to 6 weeks -> Modal to confirm final cadence change -> new billing and shipping date is still Sep 8, and frequency is updated after the shipping date."
							<p> "The intent behind this decision making was two fold. Whilst we would use the same backend logic as the previously launched experiment (keeping it constrained), in fact with just a change of conditional statements, this would also match up with a patients’ mental model of editing their frequency, as validated by users during testing."
							<p> "That’s why, it is always always important to talk and validate with the end user. If one goes into testing, attached to a solution at hand, it leads to blind spots being uncovered by the user themselves."
							<p> "Validating with users across two rounds of testing here helped us de-risk an earlier assumption that users’ might prefer a change to their subscription only after their recent box came in, and we were thus confident in shipping out this solution to a third of our cohort."
							<h3> "Scaled to desktop"
							<div[d:vflex d@md:hflex w:100% p:2]> # image-wrapper container
								# image should have a vflex only container that puts in a caption. 
								<div[d:vflex w:100% ai:center]> 
									<img[rd:xl w:100%] src=case-photos.networks-mvp> # w@md used to focus single image in the center for larger screens.
									<p[c:cool5 fw:300 margin-block:12px]> "Desktop designs"



						<div[d:flex fld:column pt:0 w:100%]> # Results and impact
							<h2> "Result and Impact"
							<p> "We released the initial feature to 1/3 of all Curology patients for the first three months, before releasing to the full user base. Update: Subscription cadence is now a permanent feature within the Curology subscription experience."
							<p> "Relative lift of 25% retention in cohort that changed frequency at least once."
							<p> "10% relative lift in cohort that stayed on default frequency."
						...
						<div[d:flex fld:column pt:0 w:100%]> # Reflections
							...
							<p> "Looking back, however, one thing I would have loved to engage in was deeper multi-variate testing. To de-risk our assumptions and move towards data informed design, it would have been great to test out our solution within the 2nd user test to a broader set of users."
							<p> "In essence, it would have provided statistical significance in terms of feature usage, as well as making certain which solution is more successful with users in terms of retention."
							<p> "Even though we had validated with users during user testing, that our final solution was matching a users’ mental model, the certainty of a multi-variate, or even two-variant test with a larger sample size would have provided more certain evidence that the solution we went with, was the right one. This is especially true as it pertains to “feature usage over time”, and whether there would be a significant delta between the two variants."
